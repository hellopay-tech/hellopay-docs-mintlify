...
An open payment link can offer Nequi when it is enabled for your account. The `rail` restriction on `POST /payment-links` currently accepts only `PSE` or `BRE_B`; omit `rail` to let the payer choose Nequi in the hosted checkout.
...
| `pse.bank` | `string` | No | PSE bank selected by the payer. When `pse` is sent, `bank` must be present. Use one of the supported bank codes from the [PSE payin guide](/guides/payin-pse#supported-banks). If omitted, the payer selects the bank in the hosted checkout. |
