...
3. The payer completes the authorization through the PSE bank flow.
4. HelloPay sends webhook events such as `payin.confirmed`, `payin.canceled`, or `payin.declined`.
5. After the payer finishes the flow, HelloPay redirects them to your `callbackUrl`.
...
- `pse.personType`: payer type. Supported values are `NATURAL` and `BUSINESS`.
...
| Value | Description |
| --- | --- |
| `NATURAL` | Individual person paying with a personal bank account. |
...
- `sourceData` echoes the PSE-specific data associated with the payin, including the selected bank and person type.
