...
No real money moves in sandbox. HelloPay does not connect directly to real payment methods such as PSE, BRE-B, or Nequi in this environment. Instead, it simulates the expected production behavior so you can validate your integration end-to-end.
...
- In `NEQUI` payins, `sourceData.phone` contains the payer's mobile number. No redirect URL or QR code is generated.
- For a Nequi sandbox payin, use `1000000001` to test confirmation and verify that `sourceData.phone` matches the submitted number.
